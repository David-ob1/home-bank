...
public class HomebankingApplication {	//esto es el main donde se trabaja
	...
	public CommandLineRunner initData(ClientRepository clientRepository, AccountRepository accountRepository, TransactionRepository transactionRepository, LoanRepository loanRepository, ClientLoanRepository clientLoanRepository,CardRepository cardRepository ){
		return  args ->{
			...
			Account account1 = new Account("VIN001", LocalDate.now(),20000,true,SAVING);
			...
			Transaction transactionC1A = new Transaction(DEBIT, -1500,"shop",LocalDateTime.now(),account1.getBalance(),true);
			account1.addTransaction(transactionC1A);
			transactionRepository.save(transactionC1A);

			// transaccion cliente 1, cuenta 1
			Transaction transactionC1B = new Transaction(CREDIT, 900,"salary",LocalDateTime.now(),account1.getBalance(),true);
			account1.addTransaction(transactionC1B);
			transactionRepository.save(transactionC1B);

			//transaccion cliente 1,cuenta 1
			Transaction transactionC1C = new Transaction(DEBIT, -350,"AMAZON, phone",LocalDateTime.now(),account1.getBalance(),true);
...
//			ClientLoan client2LoanB = new ClientLoan(200000, 36);
			...
			ClientLoan client1LoanA = new ClientLoan(400000 * (1 + loanA.getInterest()), 60 ,400000 * (1 + loanA.getInterest()),60);

				//completar antes de la guillotina
//			ClientLoan client1LoanB = new ClientLoan(50000, 12);
//
//			ClientLoan client2LoanA = new ClientLoan(100000,24);
//			ClientLoan client2LoanB = new ClientLoan(200000, 36);
...
//				editar esta parte para hacerla funcional antes q gullie nos mate
